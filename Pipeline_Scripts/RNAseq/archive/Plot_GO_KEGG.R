...
#p set = 0.05
#run in directory including two files:*_GO_Enrichment.txt *_KEGG_Enrichment.txt
...
GROUPS <- args[1] # FC2vsFD4
pSet <- 0.05

GO_Name = paste(GROUPS, "_GO_Enrichment.txt", sep="")

KEGG_Name = paste(GROUPS, "_KEGG_Enrichment.txt", sep="")
...
GOFile.three <- GOFile.three[GOFile.three$pvalue<pSet,]
...
#如果Description有重复，第二个加上一个句号，以区别
# GOFile.three$Description[duplicated(GOFile.three$Description)]<- paste(GOFile.three$Description[duplicated(GOFile.three$Description)],".",sep="")
...
# plotting
p = ggplot(GOFile.three, aes(x = Description, y = Count)) + 
...
# output pdf and png
WhetherLong= (sum(nchar(as.character(GOFile.three$Description)) > 45)>0)  
if (WhetherLong) {
    filename  = paste(GROUPS,"_GO_barplot.pdf",sep="")
    ggsave(file=filename,p, width=15, height=10, units="in")

    filename = paste(GROUPS,"_GO_barplot.png",sep="")
    ggsave(file=filename,p, width=15, height=10, units="in")
}  else {                  
    filename  = paste(GROUPS,"_GO_barplot.pdf",sep="")
    ggsave(file=filename,p, width=10, height=10, units="in")
...
kegg_list <- kegg_list[kegg_list$pvalue<pSet,]
