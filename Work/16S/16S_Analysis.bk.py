...
def PickOTU(AllSeqFile, MetaData, gg_13_8_otus, OutPut, ParamsFile):
    ...
    ### pick otu ###
    Command3 = "pick_open_reference_otus.py -i %s -r %s -o %s -s 0.1 -m usearch61 -p %s \n\n" % (AllSeqFile, gg_13_8_otus, OTU_Dir, ParamsFile)
    ...
    Command6 = "summarize_taxa_through_plots.py -i %s -m %s -o %s" % (OTU_Table_biom, MetaData, Taxa_Dir)
    print "--** OTU summarize taxa and plot **--Command6: \n%s" % Command6
    os.system(Command6)
...
def Alpha_Diversity(OutPut):
    ...
    Command13 = "alpha_diversity.py -i %s -m observed_species,shannon,PD_whole_tree,singles,simpson,observed_otus,chao1 -o %s -t %s" % (OTU_Table_biom, alpha_diversity_Dir, rep_set_tre)
    print "--** aplha diversity **--Command13: \n%s\n\n" % Command13
    os.system(Command13)
