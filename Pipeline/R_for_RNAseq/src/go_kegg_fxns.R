...
##' get GO annotation and enrichment results using clusterProfiler.
##'
##'
##' @title getGOResults
##' @param 
...
##' @example 
##' @author Yubing Li
getGOResult <- function(sigfiles, pSet, output_go_file)  {
    gene_list <- read.table(sigfiles,row.names=1,sep = '\t',header = T,stringsAsFactors=F)
    ...
    write.table(ego_three, output_go_file, quote=F,row.names = F,sep = "\t")
}
